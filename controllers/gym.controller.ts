...
export class GymController {
    ...
    async create(req: Request, res: Response) {
        ...
        if (
            ...
            res.status(400).json({
                error: "Missing required fields: name, address, contact, description, capacity, equipments, activities, ownerId",
            });
            return;
        }
    ...
    async update(req: Request, res: Response) {
        ...
        const gym = await this.gymModel.findByIdAndUpdate(gymId, updates, { new: true });
    ...
    async updateStatus(req: Request, res: Response) {
        ...
        if (!Object.values(GymStatus).includes(status)) {
            res.status(400).json({
                error: "Invalid status. Must be one of: " + Object.values(GymStatus).join(", "),
            });
            return;
        }

        const gym = await this.gymModel.findByIdAndUpdate(gymId, { status }, { new: true });
